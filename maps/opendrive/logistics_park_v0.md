...
J10/J20 的直行连接道由 5 m 扩展到 23.5 m，覆盖半径 20 m、宽 3.5 m 的转弯通道展开范围。中间普通道路改为从 X=73.5 m 到 X=146.5 m，长度 73 m；主路整体行驶轨迹和连接拓扑不变。

单车道转弯连接道的左侧边界延续相邻道路外轮廓，保留为实体实线；右侧边界伸入主路内部，使用 `roadMark type="none"`，反向转换时对应 `virtual_boundary`。虚拟边界只表达路口内部通行通道，不代表真实漆线。当前版本未单独建模更宽的路口路面 Polygon。
